...
def gk_expect_mixed_logistic(Z_k_Ybar, S_11, S_12, S_21, S_22, L, sigma_sq, alpha):
    """ Compute E[Z|Zk, Ybar] for the mixed logistic regression model. """
    Sigma_k = np.block([[S_11, S_12], [S_21, S_22]])
    Zk = Z_k_Ybar[:L]
    Y = Z_k_Ybar[-1]
    return compute_E_Z_given_Zk_Y(Zk, Y, Sigma_k, alpha)

def compute_P_c_given_Zk_Y(Zk, Y, alpha, Sigma_k):
    result = np.zeros_like(alpha)
    for i, a in enumerate(alpha):
        result[i] = a * compute_P_Y_given_Zk_c(Y, Zk, i, alpha, Sigma_k)
    return result / norm(result, 1)


def compute_P_Y_given_Zk_c(Y, Zk, c, alpha, Sigma_k):
    L = len(alpha)
...
def compute_E_Zi_given_Y_Zk_c(i, Y, Zk, c, Sigma_k, alpha):
    L = len(alpha)
    mu_zzk = Sigma_k[:L, L:] @ pinv(Sigma_k[L:, L:]) @ Zk
    Sigma_zzk = Sigma_k[:L, :L] - Sigma_k[:L, L:] @ pinv(Sigma_k[L:, L:]) @ Sigma_k[L:, :L]
    mu_i_k = mu_zzk[i]
    ...
    P_Y_1_given_Zk_c = compute_P_Y_given_Zk_c(1, Zk, j, alpha, Sigma_k)
    if Y == 1:
...
def compute_E_Z_given_Zk_Y(Zk, Y, Sigma_k, alpha):
    ...
    P_c_given_Zk_Y = compute_P_c_given_Zk_Y(Zk, Y, alpha, Sigma_k)
    for j in range(L):
        E_Z_given_Y_Zk_c = np.array([compute_E_Zi_given_Y_Zk_c(i, Y, Zk, j, Sigma_k, alpha) for i in range(L)])
...
def run_mixed_logistic_trial(p, L, n, alpha, B_row_cov, n_iters, RNG=None):
    ...
    B_hat_list, M_k_B_list = matrix_GAMP(X, Y, B_hat_0, B_row_cov, 0, alpha, n_iters, gk_expect_mixed_logistic)
