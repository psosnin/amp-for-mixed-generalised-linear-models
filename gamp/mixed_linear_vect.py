...
def apply_gk_mixed_linear(Theta_k, Y, S_11, S_12, S_21, S_22, L, sigma_sq, alpha):
    ...
    for i in range(L):
        cov_ZK_Ybar_and_Z_given_cbar[i, :, :] = np.hstack((S_12, S_11[i, :][:, None]))
    ...
    E_Z_given_Zk_Ybar_cbar = cov_ZK_Ybar_and_Z_given_cbar @ pinv(cov_Zk_Ybar_given_cbar) @ Theta_k_Ybar.T
    ...
    P_cbar_given_Zk_Ybar[:, np.all(P_cbar_given_Zk_Ybar == 0, axis=0)] = 1  # handle singular S_a (all zero pdf)
