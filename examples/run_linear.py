...
import context
from gamp.models.linear import run_linear_trial
...
for i in range(n_trials):
    beta, beta_hat_list, mu_k_list = run_linear_trial(
        p, n, sigma_sq, sigma_beta_sq, n_iters)
    mse[i, :] = np.array([beta_loss(beta, beta_hat)
                         for beta_hat in beta_hat_list])
    mse_se[i, 1:] = np.array([state_evolution_mse(
        mu, mu, sigma_beta_sq) for mu in mu_k_list])
